...
import { AttemptHistoryEntry, Difficulty, Rating, ReviewMode, StudyState, UserSettings } from "./types";
import { getFsrsCard, getFsrsScheduler, serializeFsrsCard, toFsrsRating } from "./studyState";
...
export function applyReview(input: ApplyReviewInput): StudyState {
  ...
  const nextCard = scheduler.repeat(currentCard, new Date(now))[toFsrsRating(input.rating)].card;

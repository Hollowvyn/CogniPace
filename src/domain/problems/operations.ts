/**
 * Pure entity behaviours for Problem. Repositories delegate to these so
 * the data layer never re-implements domain rules.
 *
 * `applyEdit` records that the user has touched a field (sticky edits).
 * `mergeImported` is the inverse — it patches a Problem with newly
 * imported data while preserving any field the user has manually edited.
 */
import type {
